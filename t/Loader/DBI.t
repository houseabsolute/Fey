...
for my $table1 ( grep { $_->name() ne 'TestView' } $schema1->tables() )
{
    ...
    compare_pk( $table1, $table2 );
    compare_columns( $table1, $table2 );
    compare_fks( $table1, $table2 );
}
...
sub compare_fks
{
    ...
    my %fk1 = map { _fk_id($_) => 1 } $schema1->foreign_keys_for_table($table1);
    my %fk2 = map { _fk_id($_) => 1 } $schema2->foreign_keys_for_table($table2);
